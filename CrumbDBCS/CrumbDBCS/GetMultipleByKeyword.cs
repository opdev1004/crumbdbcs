...
namespace CrumbDBCS
{
    ...
    {
        public async Task<MultipleData> GetMultipleByKeyword(string dirname, string databasename, string collectionname, string keyword, int position, int count, Encoding? encoding = null)
        {
			...
			try
            {
				...
				try
				{
					while (enumerator.MoveNext())
					{
						...
						if (!basename.Contains(keyword, StringComparison.OrdinalIgnoreCase)) continue;
